...
class ProfileLauncher:
    ...
    def get_profile_display_name(self, platform: str, profile_name: str) -> str:
    ...
    def get_profile_icon_path(self, platform: str, profile_name: str) -> Path | None:
        ...
        if not path.is_absolute():
            path = self.get_profile_dir(platform, profile_name) / path
    ...
    def create_profile(self, platform: str, profile_name: str, homepage: str | None = None) -> Path:
        ...
        if homepage:
            ...
            if created and platform == STATIC_PLATFORM:
                # Atualiza somente a conta nova nos workers compartilhados.
    ...
    def set_profile_display_name(self, platform: str, profile_name: str, display_name: str):
        data = self._read_profile_data(platform, profile_name)
        data["display_name"] = display_name
        self._write_profile_data(platform, profile_name, data)

    def enqueue_static_platform_favicons(self, *, force=False, on_progress=None, on_complete=None) -> bool:
        with self._favicon_lock:
            ...
            for profile_name in profiles:
                if self._enqueue_static_profile_favicon_locked(profile_name, force, batch_id):
                    queued += 1
    ...
    def enqueue_static_profile_favicon(self, profile_name: str, *, force=False, on_complete=None) -> bool:
        with self._favicon_lock:
            batch_id = object()
            self._favicon_batches[batch_id] = {
                "remaining": 0,
            ...
            queued = self._enqueue_static_profile_favicon_locked(profile_name, force, batch_id)
            self._favicon_batches[batch_id]["remaining"] = 1 if queued else 0

        if not queued:
            self._finish_favicon_batch(batch_id)
        return queued

    def _enqueue_static_profile_favicon_locked(self, profile_name: str, force: bool, batch_id) -> bool:
        key = (STATIC_PLATFORM, profile_name.lower())
        if key in self._favicon_pending:
            return False
        self._favicon_pending.add(key)
        self._favicon_queue.put((profile_name, force, batch_id, key))
        return True

    def _favicon_worker(self):
        while True:
            profile_name, force, batch_id, key = self._favicon_queue.get()
            try:
                status = self.sync_static_profile_favicon(profile_name, force=force)
            except Exception as error:
                print(f"Aviso: falha inesperada ao atualizar favicon de {profile_name}: {error}")
                status = "failed"
            finally:
                with self._favicon_lock:
    ...
    def _record_favicon_result(self, batch_id, profile_name: str, status: str):
        ...
        with self._favicon_lock:
            ...
            if batch is None:
                return
            batch["result"][status] = batch["result"].get(status, 0) + 1
            batch["remaining"] -= 1
    ...
    def _finish_favicon_batch(self, batch_id):
        ...
        result = None
        with self._favicon_lock:
            batch = self._favicon_batches.pop(batch_id, None)
            if batch is None:
                return
            if batch.get("global"):
                self._favicon_global_running = False
    ...
    def sync_static_profile_favicon(self, profile_name: str, *, force=False) -> str:
        data = self._read_profile_data(STATIC_PLATFORM, profile_name)
        homepage = data.get("homepage")
        normalized_homepage = self._normalize_favicon_homepage(homepage)
        if normalized_homepage is None:
            return "skipped"
        if not force and self._profile_has_valid_favicon(STATIC_PLATFORM, profile_name, data):
            return "skipped"

        favicon = self._download_favicon(normalized_homepage)
        if favicon is None:
            return "failed"

        content, extension = favicon
        profile_dir = self.get_profile_dir(STATIC_PLATFORM, profile_name)
        icon_file = profile_dir / f"navehub_favicon{extension}"
        tmp_file = profile_dir / f".navehub_favicon{extension}.tmp"
        try:
            tmp_file.write_bytes(content)
            os.replace(tmp_file, icon_file)
        except OSError:
            try:
        ...
        self._write_profile_data(STATIC_PLATFORM, profile_name, data)
        return "updated"

    def _profile_has_valid_favicon(self, platform: str, profile_name: str, data: dict | None = None) -> bool:
        if data is None:
            data = self._read_profile_data(platform, profile_name)
        icon_path = data.get("icon_path") or data.get("account_icon")
        if not isinstance(icon_path, str) or not icon_path.strip():
    ...
    @staticmethod
    def _normalize_favicon_homepage(homepage) -> str | None:
    ...
    def _download_favicon(self, homepage: str) -> tuple[bytes, str] | None:
        candidates = self._favicon_candidates(homepage)
        for favicon_url in candidates:
            response_data = self._fetch_url(favicon_url)
            if response_data is None:
                continue
            content, content_type = response_data
            if not self._is_favicon_payload(favicon_url, content, content_type):
                continue
            return content, self._favicon_extension(favicon_url, content_type)
        return None

    @staticmethod
    def _is_favicon_payload(favicon_url: str, content: bytes, content_type: str) -> bool:
        """Evita persistir uma página HTML como se fosse um favicon."""
        if not content or "text/html" in content_type.lower():
            return False
        if content.startswith((b"<!DOCTYPE", b"<html", b"<HTML")):
            return False
        if content.startswith(b"\x89PNG\r\n\x1a\n"):
            return True
        if content.startswith(b"\xff\xd8\xff"):
            return True
        if content.startswith((b"GIF87a", b"GIF89a", b"RIFF")):
            return True
        if content[:4] == b"\x00\x00\x01\x00":
            return True
        return any(
            marker in content_type.lower()
            for marker in ("image/", "icon")
        )

    def _favicon_candidates(self, homepage: str) -> list[str]:
        parsed = urllib.parse.urlparse(homepage)
        origin = f"{parsed.scheme}://{parsed.netloc}"
        candidates = []
        response_data = self._fetch_url(homepage)
        html = response_data[0].decode("utf-8", errors="ignore") if response_data else ""
        ...
        if html:
            ...
            for href in parser.hrefs:
                candidates.append(urllib.parse.urljoin(homepage, href))

        candidates.extend([
            urllib.parse.urljoin(origin, "/favicon.ico"),
            f"https://www.google.com/s2/favicons?domain={urllib.parse.quote(parsed.netloc)}&sz=64",
        ])
    ...
    def _fetch_url(self, url: str) -> tuple[bytes, str] | None:
        for attempt in range(FAVICON_RETRIES + 1):
            try:
                request = urllib.request.Request(
                    url,
                    headers={"User-Agent": "NaveHub/1.4"},
                )
                with urllib.request.urlopen(request, timeout=FAVICON_REQUEST_TIMEOUT) as response:
                    content = response.read(FAVICON_MAX_BYTES)
    ...
    @staticmethod
    def _favicon_extension(favicon_url: str, content_type: str) -> str:
        content_type = content_type.lower()
        if "png" in content_type:
            return ".png"
        if "jpeg" in content_type or "jpg" in content_type:
            return ".jpg"
        suffix = Path(urllib.parse.urlparse(favicon_url).path).suffix.lower()
        if suffix in {".ico", ".png", ".jpg", ".jpeg", ".webp", ".gif"}:
    ...
    def ensure_v14_accounts(self):
        ...
        created_names = []
        for display_name, homepage in V14_STATIC_PLATFORM_ACCOUNTS:
            if self._create_seed_profile(STATIC_PLATFORM, display_name, homepage):
                created_names.append(display_name)

        if created_names:
            self._append_profiles_to_order(STATIC_PLATFORM, created_names)
        self._sync_static_platform_order()

        try:
            (self.base_dir / V14_SEED_MARKER).touch(exist_ok=True)
        except OSError as error:
            print(f"Aviso: não foi possível registrar contas v1.4: {error}")

    def _remove_legacy_static_platform_dirs(self):
        for platform in LEGACY_STATIC_PLATFORMS:
            legacy_dir = self.base_dir / platform.lower().replace(" ", "")
            current_dir = self.base_dir / STATIC_PLATFORM.lower().replace(" ", "")
            if legacy_dir == current_dir or not legacy_dir.exists():
                continue
            try:
                shutil.rmtree(legacy_dir)
            except OSError as error:
                print(f"Aviso: não foi possível remover categoria antiga {platform}: {error}")

    def _expected_static_profile_names(self) -> list[str]:
        return [
            self.get_profile_dir(STATIC_PLATFORM, display_name).name
            for display_name, _homepage in V14_STATIC_PLATFORM_ACCOUNTS
        ]
    ...
    def _sync_static_platform_order(self):
        ...
        existing = {
            ...
            for directory in self.get_platform_dir(STATIC_PLATFORM).iterdir()
            if directory.is_dir()
        }
        order = [
            existing[name.lower()]
        ...
        self.save_profile_order(STATIC_PLATFORM, order)
    ...
    def _create_seed_profile(
        ...
        profile_dir = self.get_profile_dir(platform, display_name)
        created = not profile_dir.exists()

        profile_dir.mkdir(parents=True, exist_ok=True)
        data = self._read_profile_data(platform, display_name)
        data["display_name"] = display_name
        if homepage:
            data["homepage"] = homepage
        elif platform != STATIC_PLATFORM:
            default_homepage = self.get_default_homepage(platform)
            if default_homepage:
                data["homepage"] = default_homepage
        self._write_profile_data(platform, display_name, data)
        return created

    def _append_profiles_to_order(self, platform: str, profile_names: list[str]):
        new_storage_names = [
            self.get_profile_dir(platform, name).name
            for name in profile_names
        ]
        new_keys = {name.lower() for name in new_storage_names}
        order = self._read_profile_order(platform)
        if not order:
            order = [
                directory.name
                for directory in self.get_platform_dir(platform).iterdir()
                if directory.is_dir() and directory.name.lower() not in new_keys
            ]
        ...
        self.save_profile_order(platform, order)
    ...
    def launch_profile(self, platform: str, profile_name: str):
        ...
        title_source = build_page_title_source(
            self.get_profile_display_name(platform, profile_name)
        )
        popup_blocker_class = {
        ...
        session = (
            popup_blocker_class(process, profile_dir, cdp_port, title_source)
            if popup_blocker_class is not None
            else NaveHubTitleSession(process, profile_dir, cdp_port, title_source)
        )
